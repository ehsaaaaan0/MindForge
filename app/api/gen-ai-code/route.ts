...
function sanitizeJsonString(jsonStr: string): string {
    return jsonStr.replace(/\\(["\\/bfnrt]|u[0-9a-fA-F]{4})|\\/g, (match) => {
        if (match.length > 1) {
            return match;
        }
        return '\\\\';
    ...
    const stream = new ReadableStream({
        async start(controller) {
            ...
            try {
                ...
                const sanitized = sanitizeJsonString(accumulated);
                try {
                    parsed = JSON.parse(sanitized);
                } catch (err) {
                    console.error("[gen-ai-code] JSON parse error. Raw accumulated text:", accumulated);
                    console.error(err);
                    enqueue(
                        sseEvent("error", {
