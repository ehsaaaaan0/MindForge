...
function SandpackInner({
  ...
  // Initialize prevFilesRef with current active files to avoid redundant updates on mount
...
root.render(<React.StrictMode><App /></React.StrictMode>);`
      ...
      zip.file(
        "README.md",
        `# MindForge App\n\nGenerated with [MindForge](https://mindforge.app).\n\n## Getting started\n\n\`\`\`bash\nnpm install\nnpm start\n\`\`\``
      );
...
export function CodePanel({
  ...
  // Force a full remount when transitioning from placeholder files to real generated files,
  // but keep the key stable during inline improvements (via onImprove/onFixError)
  // to avoid resetting iframe state mid-stream.
  const filePathKey = fileData
    ? `active-${Object.keys(files).sort().join("|")}`
    : "placeholder";
